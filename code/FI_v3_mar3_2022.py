...
# POINT-IN-TIME
e_r_toronto_dec11 = np.array([0.0625,
0.0322,
0.0096,
0.0068,
0.0321,
...
0.0083,
0.0098])
N = len(e_r_toronto_dec11 )
delta_f = np.zeros(int(N*(N-1)/2))*np.nan
s=0
for i in range(N):
    for j in np.arange(i+1,N):
        delta_f[s] = np.abs(e_r_toronto_dec11[j]-e_r_toronto_dec11[i])
        s=s+1
...
plt.savefig('FI_point-in-time.png', bbox_inches = "tight", dpi=300)
...
for q in range(yorktown_qubits):
    q=str(q)
