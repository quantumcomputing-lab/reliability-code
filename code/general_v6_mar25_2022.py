...
"""
the below supprsesses warnings like (not the best way to handle):
/home/samudra/anaconda3/lib/python3.8/site-packages/IPython/core/interactiveshell.py:3263: DtypeWarning: Columns (8185,8187,8188,8191,8193,8194,8197,8199,8200,8203,8205,8206) have mixed types.Specify dtype option on import or set low_memory=False.
...
# set output dir
outputdir = '/home/samudra/Desktop/mc1_reliability/test/'

A4ratio = 297/210
tolerance = np.float64(0.05) # this is user-input - there is no correct answer
metrics = ('FI', 'FG', 'tau', 'eta')
foldername = '/home/samudra/Desktop/mc1_reliability/data/'
#'sjv_stability_feb_2022/data/'
...
def generate_metrics(devicename, nqubits, metrics, foldername, filename):
    ...
    df_all = pd.read_csv(foldername + devicename +'/' + filename)
    ...
    for metric in metrics:
        # Addressability
        if metric == 'eta':
            nmi_df = pd.read_csv(foldername+devicename+'/'+devicename+'/'+devicename+'_nmi_df.csv')
        ...
        if metric == 'FI':
            ...
            F = pd.Series(df.iloc[-1,2:].values.astype('float'),index= df.iloc[-1,2:].index)
            F = F[~np.isnan(F)]/1000
        ...
        if metric == 'FI':
            ...
            npairs = len(last_df)
            ticks = [i for i in range(0, npairs, max(1, int(npairs/5)))]
...
def latest_time_array_FI_FG(df):
    series_s = pd.Series(df.iloc[-1,2:].values.astype('float'),index= df.iloc[-1,2:].index)
    F = remove_nan_1_0(series_s)  
    return F
...
def plot_bar(data, base, labels, xlabel, ylabel, figname, devicename):
    ...
    #plt.ylim(0, 7.5)
    #plt.xlim(0, 11)
    plt.savefig(outputdir+devicename+'/'+figname, bbox_inches = "tight", dpi=300)
    plt.clf()
...
def plot_heatmap(last_df, ticks, labels, xylabel, figname, devicename, colorbar_label):
    ...
    figure.savefig(outputdir+devicename+'/'+figname, 
                   bbox_inches = "tight", dpi=300)
    figure.clf()


def trunc_at(s, d='_', n=3):
...
def plot_raw_ts(df1, figname, devicename, time_labels):
    ...
    for q in np.arange(0, df1.shape[1]):
        #print(q)
        y_data = df1.iloc[:,q]
        y_data[np.logical_or(y_data==0, y_data==1) ]=np.nan # tau unaffacted too (but bad coding)
        ax[ax_counter].plot(x_data, y_data, color='b')
        ax[ax_counter].set_ylabel(trunc_at(df1.columns[q]), rotation=0, fontsize="12")
        ...
        if ax_counter % rows_per_sublpot == 0 or q==df1.shape[1]-1:
            ...
            fname = outputdir+devicename+'/'+ figname + str(int(q/rows_per_sublpot)) + '.png'
...
def plot_nmi_heatmap(nmi_df, nqubits, colorbar_label, xylabel, figname, devicename ):
    ...
    sns_plot.axes.set_xlabel(xylabel, fontsize=12)
    figure = sns_plot.get_figure()
    figure.savefig(outputdir+devicename+'/'+figname, bbox_inches = "tight", dpi=300)
    figure.clf()
