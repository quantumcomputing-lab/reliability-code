...
'''
for devicename in device_list.keys():
    create_nmi_df(devicename)
'''
...
if len(F)<20:
    plot_bar(data = F.values, base = np.arange(1,1+len(F.values)), labels=F.index, 
            xlabel='Point-in-time in initialization fidelity', 
            ylabel = 'Value', 
...
df1 = df.iloc[:,2:]
plot_raw_ts(df1, figname = 'FI_raw_time_', time_labels = time_labels )
...
last_df = latest_time_array_FI_FG(df)
npairs = len(last_df)
ticks = [i for i in range(0, npairs, max(1,int(nqubits/20))))]
...
# RAW DATA ACROSS TIME
df1 = df.iloc[:,2:]
plot_raw_ts(df1, figname = 'FG_raw_time_', time_labels = time_labels )

# Duty Cycle
df = tau_df(df_all, nqubits)
# CI ACROSS TIME
# this snippet below needs to subsumed
Flist=list()
for col in df.columns[2:]:
    Flist = Flist + [item for item in df[col] if not math.isnan(item)]
F = np.array(Flist)

plot_CI(F/1000, xlabel="CNOT duty cycle (x0.001)", 
        ylabel="Normalized histogram", figname = 'tau_across-time.png')
# CI POINT-IN-TIME
F = [item for item in df.iloc[-1,2:] if not math.isnan(item)]
plot_CI(np.array(F)/1000, xlabel="Inter-gate difference in duty cycle  (x0.001)", 
...
labels = [item.replace('_tau_value', '') for item in np.array( last_df.index )][::int(npairs/5)]
xlabel = 'Gate Pair #'
...
# RAW DATA ACROSS TIME
df1 = df.iloc[:,2:]
plot_raw_ts(df1, figname = 'tau_raw_time_', time_labels = time_labels )
...
def plot_bar(data, base, labels, xlabel, ylabel, figname):
...
def plot_raw_ts(df1, figname, time_labels):
    ...
    for k in np.arange(0,8):
        ...
        for i in range(rows_per_sublpot):
            ...
            if q<df1.shape[1]:
                ...
                ax[i].yaxis.set_label_coords(-0.16,0)
