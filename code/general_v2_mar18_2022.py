...
'''
gather all columns that qualify with the phrase:

TODOS:
data_update():
    - this should also run for eta
    - a flag should check if the data came back
    - if yes then report will be published; else it will publish the old report with warning
...
npairs = len(last_df)
ticks = [i for i in range(0, npairs, 35)]
...
nmi_df  = get_nmi_df(foldername+csvfile) # if you want error bars, we will need to run multiple times
colorbar_label = 'Mutual Information\n (normalized, %)'
xylabel = "Qubit #"
figname = 'nmi.png'
plot_nmi_heatmap(nmi_df, nqubits, colorbar_label, xylabel, figname ) #this needs to be combined with plot_heatmap
...
def get_nmi_df(csvfile, data_refresh=0):
    if data_refresh:
        filename = '622d13ca25b05bf21476fa4e-output.json'
        foldername = '/home/samudra/Desktop/sjv_stability_feb_2022/data/job-622d13ca25b05bf21476fa4e/'
        results_str = retrive_data_from_json(filename, foldername)
        
        qubit_data_matrix = np.zeros((8192,127))
        ...
        with open(csvfile, 'w') as csv_file:  
            ...
            writer.writerow(np.arange(0,127))
            for i in range(nqubits):
